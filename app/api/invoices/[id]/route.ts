...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (payload.action === "send") {
      const paymentTitle = existing.payment_title || buildInvoicePaymentTitle({ invoiceNumber: existing.invoice_number, dogNames: existing.dog_names ?? [], serviceName: existing.service_name || "Service", durationMinutes: existing.duration_minutes, billingDays: existing.billing_days });
      ...
      const { data, error } = await supabaseAdmin.from("portal_invoices").update({ status: "pending", payment_title: paymentTitle, payment_url: revolutOrder.checkoutUrl, revolut_order_id: revolutOrder.orderId }).eq("id", id).select("*").single();
    ...
    const updates = { client_name: cleanString(payload.clientName), client_email: cleanString(payload.clientEmail).toLowerCase(), client_phone: cleanString(payload.clientPhone), client_address: cleanString(payload.clientAddress), dog_names: dogNames, issued_on: getSafeDate(payload.issuedOn), due_on: getSafeDate(payload.dueOn), line_items: lineItems, amount_cents: getInvoiceTotalCents(lineItems, payload.amountCents), currency: (cleanString(payload.currency) || "EUR").toUpperCase(), status: (cleanString(payload.status) || existing.status || "pending") as InvoiceStatus, payment_title: cleanString(payload.paymentTitle) || buildInvoicePaymentTitle({ invoiceNumber: existing.invoice_number, dogNames, serviceName, durationMinutes, billingDays }), service_name: serviceName || null, duration_minutes: durationMinutes, billing_days: billingDays, notes: cleanString(payload.notes) || null };
