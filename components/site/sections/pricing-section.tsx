    ...
    <div className="grid gap-7 lg:grid-cols-3">
      {plans.map(
        ({
          ...
          <article
            key={title}
            className={`relative rounded-3xl bg-[#111821] p-8 shadow-sm ring-1 ${featured ? "ring-4 ring-[#d1a34d]" : "ring-white/10"}`}
          >
            {featured ? (
              <div className="absolute right-6 top-6 rounded-full bg-[#3a2915] px-4 py-1 text-xs font-extrabold uppercase tracking-wide text-[#f1c96d]">
                Popular
              </div>
            ) : null}
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#2a2118] text-[#d1a34d]">
            ...
            <p className="mt-6 text-4xl font-extrabold text-[#d1a34d]">
              {price}{" "}
            ...
            <ul className="mt-6 space-y-3 text-sm text-[#d8cab8]">
              {features.map((feature) => (
                <li key={feature} className="flex gap-2">
                  <Check
                    aria-hidden="true"
                    className="mt-0.5 h-4 w-4 shrink-0 text-[#d1a34d]"
                  />
                  {feature}
                </li>
              ))}
...
export function GuaranteeBanner() {
  return (
    <div className="mx-auto flex max-w-4xl gap-5 rounded-3xl bg-[#17130d] p-7 text-left ring-1 ring-[#d1a34d]/25">
      <ShieldCheck
        aria-hidden="true"
        className="h-10 w-10 shrink-0 text-[#d1a34d]"
      />
      <div>
