...
export function HomeHero() {
  return (
    <section className="bg-[#fefce8] px-6 py-14 sm:px-8 lg:py-20">
      <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1fr_0.95fr] lg:items-center">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-extrabold text-emerald-700 shadow-sm">
          ...
          <h1 className="mt-7 text-5xl font-extrabold leading-tight tracking-tight text-slate-950 sm:text-6xl">
            Your Dog Deserves <span className="block text-emerald-700">Jeroen & Paws</span>
          </h1>
          ...
          <dl className="mt-10 grid grid-cols-3 gap-5 border-t border-emerald-900/10 pt-6">
            {stats.map((stat) => (
              <div key={stat.label}>
                <dt className="text-3xl font-extrabold text-emerald-700 sm:text-4xl">{stat.value}</dt>
                <dd className="mt-1 text-sm font-semibold text-slate-500 sm:text-base">{stat.label}</dd>
              </div>
            ))}
          </dl>
        </div>
        <div className="relative min-h-[320px] overflow-hidden rounded-[2rem] shadow-2xl shadow-emerald-950/15 sm:min-h-[420px]">
