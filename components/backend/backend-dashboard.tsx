...
const navGroups = [
  { label: "", items: [["Dashboard", Home, true]] },
  { label: "Manage", items: [["Bookings", CalendarDays], ["Services", PawPrint], ["Dogs", Dog], ["Clients", Users], ["Walkers & Trainers", UserRound], ["Calendar", CalendarDays]] },
  { label: "Business", items: [["Invoices", FileText]] },
  { label: "Content", items: [["Photo Updates", ImageIcon], ["Reviews", Star], ["FAQ", HelpCircle]] },
    ...
    <main className="min-h-screen bg-[#fbfaf8] text-[#151124]">
      <div className="grid min-h-screen xl:grid-cols-[18rem_1fr]">
        <aside className="hidden bg-[#070a14] px-6 py-8 text-white xl:flex xl:flex-col">
          ...
          <nav className="mt-10 flex flex-1 flex-col gap-7">
            {navGroups.map((group) => (
              <div key={group.label || "main"}>
                ...
                <div className="space-y-1.5">
                  {group.items.map(([label, Icon, active]) => (
                    <button key={label} className={`flex w-full items-center gap-4 rounded-lg px-3 py-3 text-left text-sm font-semibold ${active ? "bg-[#4f2c91] text-white shadow-lg shadow-[#4f2c91]/30" : "text-white/88 hover:bg-white/8"}`}>
