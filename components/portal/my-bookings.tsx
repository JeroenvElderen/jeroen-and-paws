...
import { ArrowRight, Bell, CalendarDays, Check, Clock3, Download, Gift, ImageIcon, MapPin, PawPrint, Users } from "lucide-react";
...
export function MyBookings({ accessToken }: { accessToken?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadBookings() {
      try {
        const response = await fetch("/api/bookings?scope=portal", {
          cache: "no-store",
  ...
  const nextBooking = upcoming[0] ?? fallbackBookings[0];
  ...
  const timeline = [
    ...
    [CalendarDays, "Session day", formatBookingDate(nextBooking.startsAt, { day: "numeric", month: "long", year: "numeric" }), "", false],
  ...
  return (
    <div className="px-4 py-6 text-[#17132a] sm:px-8 lg:px-10 lg:py-10">
      <header className="mx-auto flex max-w-6xl items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-5"><button aria-label="Notifications" className="relative text-[#2f1b59]"><Bell className="size-6" /><span className="absolute right-0 top-0 size-2 rounded-full bg-[#8b5cf6]" /></button><div className="relative size-14 overflow-hidden rounded-full ring-2 ring-[#ead9b8]"><Image src={nextBooking.coverImageUrl || "/images/dogs/ace.jpg"} alt={`${nextBooking.dogName} profile photo`} fill sizes="56px" className="object-cover" /></div></div>
      </header>

      <div className="mx-auto mt-10 max-w-6xl">
        {(isLoading || isFallback) && <p className="mb-5 rounded-xl border border-[#24163f]/10 bg-white px-5 py-4 text-sm text-[#665d70]">{isLoading ? "Loading live bookings…" : "Showing fallback bookings until Supabase portal rows are available."}</p>}
        ...
        <BookingCard className="mt-5">
          <div className="grid lg:grid-cols-[2fr_2.4fr_1.25fr]">
            ...
            <div className="p-8 lg:p-10">
              ...
              <div className="mt-9 flex flex-wrap gap-4"><a href={`/api/calendar/booking/${nextBooking.id}`} className="inline-flex items-center gap-2 rounded bg-[#4d2e91] px-7 py-4 text-xs font-black uppercase tracking-[0.16em] text-white"><Download className="size-4" /> Add to calendar</a><a href="#" className="rounded border border-[#4d2e91]/35 px-7 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#3f2581]">Reschedule</a></div>
            </div>
